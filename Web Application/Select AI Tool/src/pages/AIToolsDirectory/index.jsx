...
const AIToolsDirectory = () => {
  ...
  // NEW: featured is now part of the unified filter state instead of a
  // one-off fetch, so it always goes through the same rating-aware pipeline.
  ...
  // Fetch filter chip options once
  useEffect(() => {
  ...
  /**
   * SINGLE source of truth for fetching + displaying tools.
   * This is the ONLY effect that populates `aiTools`, so there is no
   * race condition anymore between "load all tools" and "load filtered
   * tools" — both paths go through here and both always attach `rating`.
   *
   * It re-runs whenever any filter changes, AND on initial mount
   * (because all the filter states have real initial values, React
   * runs this effect once automatically on first render).
   */
  useEffect(() => {
    // If there's an active search query, let the debounced search effect
    // below own the fetch instead (avoids both effects fighting).
    ...
    const fetchFilteredTools = async () => {
      ...
      try {
        ...
        let tools = res.data.map((tool) => ({
          ...tool,
          // Always attach a numeric rating right away so the UI never
          // renders a blank/undefined rating on first paint.
          rating:
    ...
    fetchFilteredTools();
  }, [selectedCategories, selectedProfession, selectedPricing, selectedRatings, featuredOnly, searchQuery]);

  // Debounced search — also attaches rating so cards never go blank.
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      if (!searchQuery) return; // handled by the effect above
      fetchSearchResults();
    }, 300);
  ...
  const handleCategoryChipClick = (chipId, chipData, newSelected) => {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Find the perfect AI tool for any job. Browse 3000+ curated AI-powered tools with advanced search, filtering by category, profession, and ratings. Compare features and pricing."
        />
          ...
          <div className="w-full max-w-[1728px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav className="flex justify-between items-center mb-16">
              <div className="flex items-center gap-2">
                ...
                <span className="text-2xl font-medium text-white font-['Public_Sans']">Free mode</span>
                <div
                  onClick={() => {
                    setFreeMode(!freeMode);
                    setSelectedPricing(!freeMode ? "Free" : "");
                  }}
            ...
            {modalOpen && (
              ...
              >
                ...
                >
                  <h2 className="text-center text-2xl font-extrabold mb-6 text-[#4f46e5]">Contact Us</h2>
            ...
            <div className="text-center mb-20 sticky top-0 z-40 px-4 py-4 sm:py-6">
              <h1
                className="text-[28px] sm:text-[28px] md:text-[36px] lg:text-[48px] xl:text-[56px]
            ...
            <div className="flex flex-col lg:flex-row gap-12">
              ...
              <aside className="w-full lg:w-[22%] lg:mt-1.5">
                <div className="bg-transparent">
                  ...
                  {/* Category Section */}
                  <div className="mb-8">
                    <h3 className="text-xl font-bold text-text-secondary font-['Lexend_Mega'] capitalize mb-5">
                      Category
                    </h3>
                    <LocalChipView
                  ...
                  {/* Profession Section */}
                  <div className="mb-8">
                    <h3 className="text-xl font-bold text-text-secondary font-['Lexend_Mega'] capitalize mb-5.5">
                      Profession
                    </h3>
                    <LocalChipView
                    ...
                    <div className="space-y-3.5">
                      {ratings.map((rating) => (
                        ...
                        >
                          ...
                          >
                            {selectedRatings.includes(rating.value) && (
                              <img src="/images/img_lucide_check.svg" alt="checked" className="w-3.5 h-3.5" />
                            )}
                          </div>
                ...
                <div className="mb-9">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-3">
                    ...
                    <div className="flex items-center gap-10 relative">
                      <span className="text-lg font-semibold text-text-primary font-['Public_Sans']">Sort By:</span>
                    ...
                    aiTools.slice(0, visibleCount).map((tool) => (
                      ...
                      >
                        <div className="flex items-start gap-3 mb-3">
                          {tool.image_url && (
                            <img src={tool.image_url} alt={tool.name} className="w-[70px] h-[70px] flex-shrink-0 rounded-lg" />
                          )}
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              window.open(tool.official_link, "_blank");
                            }}
        ...
        {showScroll && (
          ...
          >
            <img
              ...
              alt="Scroll to top"
              className="w-[46px] h-[46px] cursor-pointer hover:opacity-80 transition-opacity"
            />
          </button>
        )}
      </main>
